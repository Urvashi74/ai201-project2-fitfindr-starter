...
**Step 1:**
The agent calls `search_listings(description="vintage graphic tee", size="M", max_price=30.0)`. It scans `listings.json`, filtering by size match ("M" or "S/M") and price ≤ 30, then scores results by how many `style_tags` and `description` words overlap with "vintage graphic tee". It returns a ranked list of matches — or an error message if none are found, at which point the agent stops and tells the user what to refine.
